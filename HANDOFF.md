...
Обновлено: 2026-06-16 (v1 + Torch student prototype готов)
...
**Результат #T1–#T3:** `rel_holdout ≈ 1.033 / 1.033 / 1.004` — **≈ Mojo #5/#6/#6b**; underfit-плато подтверждено. Wall time ~6–7 с vs ~45–63 с Mojo GPU.

**Не сделано в prototype (намеренно):** v1b, 116k chains, checkpoint export, замена Mojo train path.
...
| README sync | устарел |
| ~~Torch student prototype~~ | ✅ этапы 0–2; см. § ниже |
| Torch v1b / H↑ / 50 ep | исследование — следующий шаг |
| `test_gradcheck_tiny.py` в gate | optional; файл есть, не в `test-torch-parity` |
...
- **Torch v1b / H↑ / 50 ep** — быстрый A/B в Torch (`make train-torch`); порт в Mojo только если `rel` заметно < 1
...
**Статус: готов для Phase 1 R&D** (этапы 0–2 ✅, 2026-06-16).

**Зачем:** Phase 1 упёрлась в underfit (`rel ≈ 1`), не в Mojo. Новая arch (v1b, H↑, init, schedule) в Torch — часы, в Mojo — дни (kernels + grad tests). Prototype **не** меняет production path: Mojo остаётся source of truth для deploy и `make test`.

**Сделано:**

- Одна chain: `L00_N0000.bin`, holdout @ seed 42 (3687 / 409)
- Student v0 / v1 — та же семантика, что `docs/architecture.md` и Mojo `BatchMicroNet`
- Метрики: `train_mse`, `holdout_mse`, `rel_holdout = MSE / Var(Y_holdout)`
- Env: `CALIBER158_*` через `StudentEnv`
- Прогоны **#T1–#T3** в `lerning_compare.md`
- Parity gate: `make test-torch-parity` (holdout + 1-batch loss vs Mojo CPU)

**Осталось (следующие эксперименты, не блокер prototype):**

- **v1b** linear skip — Torch first, порт в Mojo если `rel` падает
- **50 ep v1**, **H↑** (768+) — быстрые прогоны в Torch
- Optional: подключить `test_gradcheck_tiny.py` к gate
- 116k chains, worker pool, checkpoint export — вне scope prototype
...
| Arch | `CALIBER158_ARCH=v0\|v1` | те же forward graphs |
...
#### Этап 3 — v1b (следующий эксперимент)

- [ ] Linear skip `out = α·head(h1) + β·(w_res·x)` (или `ArchKind.v1b` в обоих стеках)
- [ ] A/B в Torch → если `rel` падает, порт в Mojo

#### Этап 4 — Optional hardening (частично)
...
1. Torch v1b / H↑ / 50 ep — **исследование** (дни)
2. Если `rel` заметно < 1 — **порт** winning arch в Mojo + `test-grad-*`
3. Mojo остаётся для GPU kernels, checkpoint, Phase 2 scale
