...
from .ternary_matmul import vector_add_inplace_kernel
...
def backward_head_step_kernel(
    ...
    dL_dh: UnsafePointer[Float32, MutAnyOrigin],
    batch_size: Int,
    hidden_dim: Int,
    inv_batch: Float32,
):
    """head_partial[b,j] and dL_dh[b,j] from head step."""
    ...
    dL_dh[idx] = dL_dy_tern * Float32(head_tern[j])


def backward_head_step_f32_kernel(
    err: UnsafePointer[Float32, MutAnyOrigin],
    hidden: UnsafePointer[Float32, MutAnyOrigin],
    alpha: UnsafePointer[Float32, MutAnyOrigin],
    ...
    """FP32 head weights: head_partial and dL_dh."""
    var idx = Int(block_idx.x * block_dim.x + thread_idx.x)
    var total = batch_size * hidden_dim
    if idx >= total:
        return
    ...
    var h_base = b * hidden_dim
    head_partial[idx] = dL_dy_tern * hidden[h_base + j]
    dL_dh[idx] = dL_dy_tern * head_shadow[j]


def backward_gate_up_from_dL_dh_kernel(
    dL_dh: UnsafePointer[Float32, MutAnyOrigin],
    ...
    """gate/up partial[b,j,i] from precomputed dL_dh and forward activations."""
    var idx = Int(block_idx.x * block_dim.x + thread_idx.x)
    ...
    var silu_gate = _silu(gate_j)

    var dL_dh_j = dL_dh[act_base + j]
    var dL_dgate = dL_dh_j * up_j * _silu_derivative(gate_j)
    var dL_dup = dL_dh_j * silu_gate
...
def backward_block2_input_grad_kernel(
    dL_dh1: UnsafePointer[Float32, MutAnyOrigin],
    gate2_act: UnsafePointer[Float32, MutAnyOrigin],
    up2_act: UnsafePointer[Float32, MutAnyOrigin],
    gate2_tern: UnsafePointer[Int8, MutAnyOrigin],
    up2_tern: UnsafePointer[Int8, MutAnyOrigin],
    dL_dh0: UnsafePointer[Float32, MutAnyOrigin],
    batch_size: Int,
    hidden_dim: Int,
):
    """Input grad dL_dh0[b,i] from block2 SwiGLU (no skip)."""
    var idx = Int(block_idx.x * block_dim.x + thread_idx.x)
    var total = batch_size * hidden_dim
    if idx >= total:
        return
    ...
    for j in range(hidden_dim):
        var dL_dh2 = dL_dh1[act_base + j]
        var gate_j = gate2_act[act_base + j]
        var up_j = up2_act[act_base + j]
        var silu_gate = _silu(gate_j)
        var dL_dgate = dL_dh2 * up_j * _silu_derivative(gate_j)
        var dL_dup = dL_dh2 * silu_gate
        var row = j * hidden_dim
        acc += dL_dgate * Float32(gate2_tern[row + i]) + dL_dup * Float32(up2_tern[row + i])
    dL_dh0[idx] = acc


def backward_block2_input_grad_f32_kernel(
    dL_dh1: UnsafePointer[Float32, MutAnyOrigin],
    gate2_act: UnsafePointer[Float32, MutAnyOrigin],
    up2_act: UnsafePointer[Float32, MutAnyOrigin],
    gate2_shadow: UnsafePointer[Float32, MutAnyOrigin],
    up2_shadow: UnsafePointer[Float32, MutAnyOrigin],
    dL_dh0: UnsafePointer[Float32, MutAnyOrigin],
    batch_size: Int,
    hidden_dim: Int,
):
    """FP32 block2 input grad into dL_dh0."""
    var idx = Int(block_idx.x * block_dim.x + thread_idx.x)
    var total = batch_size * hidden_dim
    if idx >= total:
        return
    ...
    for j in range(hidden_dim):
        ...
        var dL_dup = dL_dh2 * silu_gate
        var row = j * hidden_dim
        acc += dL_dgate * gate2_shadow[row + i] + dL_dup * up2_shadow[row + i]
    dL_dh0[idx] = acc
...
def enqueue_backward(
    ...
    mut dL_dh_dev: DeviceBuffer[DType.float32],
    batch_size: Int,
    hidden_dim: Int,
    input_dim: Int,
) raises -> None:
    ...
    ctx.enqueue_function[backward_head_step_kernel, backward_head_step_kernel](
        err_dev.unsafe_ptr(),
        hidden_dev.unsafe_ptr(),
        alpha_dev.unsafe_ptr(),
        head_tern_dev.unsafe_ptr(),
        grad_head_partial_dev.unsafe_ptr(),
        dL_dh_dev.unsafe_ptr(),
        batch_size,
        hidden_dim,
        inv_batch,
        grid_dim=blocks_bh,
    ...
    ctx.enqueue_function[backward_gate_up_from_dL_dh_kernel, backward_gate_up_from_dL_dh_kernel](
        dL_dh_dev.unsafe_ptr(),
        x_ptr,
...
def enqueue_backward_fp32(
    ...
    ctx.enqueue_function[backward_head_step_f32_kernel, backward_head_step_f32_kernel](
        err_dev.unsafe_ptr(),
        hidden_dev.unsafe_ptr(),
        alpha_dev.unsafe_ptr(),
...
def enqueue_backward_v1(
    ...
    y_dev: DeviceBuffer[DType.float32],
    y_batch_offset: Int,
    h0_dev: DeviceBuffer[DType.float32],
    gate_act_dev: DeviceBuffer[DType.float32],
    up_act_dev: DeviceBuffer[DType.float32],
    ...
    mut loss_scalar_dev: DeviceBuffer[DType.float32],
    mut dL_dh_dev: DeviceBuffer[DType.float32],
    mut dL_dh0_dev: DeviceBuffer[DType.float32],
    batch_size: Int,
    hidden_dim: Int,
    input_dim: Int,
) raises -> None:
    """v1 backward: head → block2 → skip → block1."""
    var inv_batch = 1.0 / Float32(batch_size)
    var bh = batch_size * hidden_dim
    var bhd = batch_size * hidden_dim * input_dim
    var bhh = batch_size * hidden_dim * hidden_dim
    var blocks_b = _ceildiv(batch_size, 256)
    var blocks_bh = _ceildiv(bh, 256)
    var blocks_bhd = _ceildiv(bhd, 256)
    var blocks_bhh = _ceildiv(bhh, 256)
    var blocks_hd = _ceildiv(hidden_dim * input_dim, 256)
    var blocks_hh = _ceildiv(hidden_dim * hidden_dim, 256)
    var blocks_h = _ceildiv(hidden_dim, 256)
    ...
    ctx.enqueue_function[backward_block2_input_grad_kernel, backward_block2_input_grad_kernel](
        dL_dh_dev.unsafe_ptr(),
        gate2_act_dev.unsafe_ptr(),
        up2_act_dev.unsafe_ptr(),
        gate2_tern_dev.unsafe_ptr(),
        up2_tern_dev.unsafe_ptr(),
        dL_dh0_dev.unsafe_ptr(),
        batch_size,
        hidden_dim,
        grid_dim=blocks_bh,
        block_dim=256,
    )
    ctx.enqueue_function[vector_add_inplace_kernel, vector_add_inplace_kernel](
        dL_dh0_dev.unsafe_ptr(),
        dL_dh_dev.unsafe_ptr(),
        bh,
        grid_dim=blocks_bh,
...
def enqueue_backward_v1_fp32(
    ...
    pred_dev: DeviceBuffer[DType.float32],
    head_shadow_dev: DeviceBuffer[DType.float32],
    gate2_shadow_dev: DeviceBuffer[DType.float32],
    up2_shadow_dev: DeviceBuffer[DType.float32],
    alpha_dev: DeviceBuffer[DType.float32],
    mut err_dev: DeviceBuffer[DType.float32],
    mut loss_partial_dev: DeviceBuffer[DType.float32],
    mut grad_alpha_partial_dev: DeviceBuffer[DType.float32],
    ...
    """v1 backward with FP32 shadow weights."""
    var inv_batch = 1.0 / Float32(batch_size)
    var bh = batch_size * hidden_dim
    var bhd = batch_size * hidden_dim * input_dim
    var bhh = batch_size * hidden_dim * hidden_dim
    ...
    ctx.enqueue_function[
        backward_block2_input_grad_f32_kernel, backward_block2_input_grad_f32_kernel
    ](
        dL_dh_dev.unsafe_ptr(),
        gate2_act_dev.unsafe_ptr(),
        up2_act_dev.unsafe_ptr(),
        gate2_shadow_dev.unsafe_ptr(),
        up2_shadow_dev.unsafe_ptr(),
        dL_dh0_dev.unsafe_ptr(),
        batch_size,
        hidden_dim,
        grid_dim=blocks_bh,
